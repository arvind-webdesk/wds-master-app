...
interface SeedDataV1 {
  ...
  branding: {
    ...
    /** Google Font family name (e.g. "Montserrat", "Open Sans"). Optional. */
...
function loadSeedData(): NormalizedSeed {
  ...
  // ── Module auto-injection (derived, not hand-edited in seed-data.json) ────
  //
  // Rules enforced by the template regardless of what the onboarding tool writes:
  //   • `dashboard`    — always enabled (all dashboardTypes render /dashboard)
  //   • `connections`, `sync-history`, `cron-sync` — enabled for 'middleware' or
  //     'custom' dashboards when at least one platform integration is turned on.
  //     Otherwise they'd point at nothing and clutter the sidebar.
  //
  // Written here instead of in seed-data.json so the rules survive regeneration
  // and future platforms (woocommerce, …) just extend the union below.
  function addIfMissing(key: string) {
    if (!enabledModules.includes(key)) enabledModules.push(key)
  }
  addIfMissing('dashboard')

  const rawDashboardType = data.dashboardType ?? 'custom'
  const supportsIntegrations =
    rawDashboardType === 'middleware' || rawDashboardType === 'custom'
  const anyPlatformEnabled =
    !!data.integrations?.shopify?.enabled ||
    !!data.integrations?.bigcommerce?.enabled
  if (supportsIntegrations && anyPlatformEnabled) {
    addIfMissing('connections')
    addIfMissing('sync-history')
    addIfMissing('cron-sync')
  }
  ...
  const integrationsAllowed =
    dashboardType === 'middleware' || dashboardType === 'custom'

  const integrations: NormalizedSeed['integrations'] = {
    shopify: {
      enabled:  integrationsAllowed && !!shopifyRaw.enabled,
    ...
    bigcommerce: {
      enabled:   integrationsAllowed && !!bigcommerceRaw.enabled,
  ...
  // Font family — optional, any non-empty string accepted.
  const rawFont = data.branding.fontFamily ?? null
  const fontFamily = typeof rawFont === 'string' && rawFont.trim().length > 0
    ? rawFont.trim()
    : null
  if (rawFont && !fontFamily) warn('branding.fontFamily is set but empty — ignoring')

  ok(`Loaded ${path.relative(REPO_ROOT, existing)} (${data.modules.length} modules, ${enabledModules.length} enabled, type=${dashboardType}${fontFamily ? `, font=${fontFamily}` : ''})`)
...
export const STATIC_CLIENT_CONFIG: StaticClientConfig = {
  ...
  fontFamily:        ${quote(data.fontFamily)},
...
// ─── Phase 2a: Strip any legacy Google Font @import block ──────────────────
// Older versions of this script wrote a `@import url('https://fonts.…')` block
// into globals.css. That conflicts with Tailwind v4's PostCSS pipeline, which
// hoists `@import "tailwindcss"` ahead of it and then errors out with
// "@import rules must precede all rules." The brand font is now loaded at
// runtime by components/shell/BrandStyle.tsx as a <link rel="stylesheet">,
// so this function only cleans up any stale block left behind.

const FONT_BEGIN = '/* ───── BEGIN onboarding-font (auto-generated, do not edit) ───── */'
const FONT_END   = '/* ───── END onboarding-font ───── */'

function writeFontBlock(_data: NormalizedSeed) {
  if (!fs.existsSync(GLOBALS_CSS_PATH)) return

  const css = fs.readFileSync(GLOBALS_CSS_PATH, 'utf8')
  const bi = css.indexOf(FONT_BEGIN)
  const ei = css.indexOf(FONT_END)

  if (bi >= 0 && ei > bi) {
    const cleaned = css.slice(0, bi) + css.slice(ei + FONT_END.length).replace(/^\n/, '')
    fs.writeFileSync(GLOBALS_CSS_PATH, cleaned, 'utf8')
    ok('Removed legacy font @import block from app/globals.css (loaded by BrandStyle.tsx)')
  } else {
    skip('Font @import block not present in app/globals.css (loaded by BrandStyle.tsx)')
  }
}

// ─── Phase 2b: Inject brand color into app/globals.css ──────────────────────
...
function writeBrandBlock(data: NormalizedSeed) {
  ...
  // Font-family overrides — only emitted when a custom font is specified.
  // The Google Fonts @import is injected separately (writeFontBlock) because
  // @import must appear before other CSS rules; the brand block is at the end.
  const fontLine = data.fontFamily
    ? `\n  --font-sans:    '${data.fontFamily}', var(--font-inter), sans-serif;\n  --font-heading: '${data.fontFamily}', var(--font-inter), sans-serif;`
    : ''
