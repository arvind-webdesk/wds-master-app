  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-base font-semibold text-foreground">System analytics</h2>
            ...
            {isLoading && !data ? (
              ...
              <ResponsiveContainer width="100%" height={256}>
                ...
                >
                  ...
                  />
                  <Bar dataKey="total" name="total" fill="var(--color-primary, oklch(0.55 0.2 250))" radius={[3, 3, 0, 0]} maxBarSize={40} />
                  <Bar dataKey="errors" name="errors" fill="var(--color-destructive, oklch(0.55 0.22 25))" radius={[3, 3, 0, 0]} maxBarSize={40} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardContent>
          ...
          <CardContent className="flex-1 px-0 pb-0">
            {isLoading && !data ? (
              ...
              <ul className="divide-y divide-border" aria-label="Recent API activity">
                {activity.map((row) => (
                  <li key={row.id} className="flex items-start gap-3 px-4 py-3 border-b border-border last:border-0">
                    <span className={`shrink-0 mt-0.5 inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${methodColor(row.method)}`}>
                      {row.method ?? '—'}
                    </span>
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-1">
                      <span className={`inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold tabular-nums ${statusColor(row.responseStatus, row.isError)}`}>
                        {row.responseStatus ?? '—'}
                      </span>
            ...
            {isLoading && !data ? (
              ...
              <ResponsiveContainer width="100%" height={224}>
                ...
                >
                  ...
                  />
                  <Area type="monotone" dataKey="total" stroke="var(--color-primary, oklch(0.55 0.2 250))" strokeWidth={2} fill="url(#signupsFill)" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </CardContent>
            ...
            {isLoading && !data ? (
              ...
              <ResponsiveContainer width="100%" height={224}>
                ...
                >
                  ...
                  />
                  <Line type="monotone" dataKey="avgMs" stroke="var(--color-primary, oklch(0.55 0.2 250))" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </CardContent>
